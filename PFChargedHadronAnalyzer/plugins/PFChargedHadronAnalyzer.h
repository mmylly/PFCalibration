...
class PFChargedHadronAnalyzer : public edm::one::EDAnalyzer<> {
 ...
 private:
  ...
  float genE_,genP_,genEta_,genPhi_;
  float pfE_,pfP_,pfEta_,pfPhi_;
  float trkP_;
  float calo_,ecal_,hcal_,rawCalo_,rawEcal_,rawHcal_;
  float trkReso_, caloReso_, w_;


  float etaEcal_,phiEcal_;
  std::vector<float> dr_,Eecal_,Ehcal_,pfcID_,pfcE_;
  ...
  double neutralHadronEnergyResolution(double clusterEnergyHCAL, double eta);

...
PFChargedHadronAnalyzer::analyze(const Event& iEvent, 
  ...
  if ( isSimu ) { 
    ...
    for( CI ci  = pfCandidates->begin(); ci!=pfCandidates->end(); ++ci)  {
      ...
      double deta = genEta_ - pfc.p4().Eta();
      double dphi = dPhi(genPhi_, pfc.p4().Phi());
      double dR   = std::sqrt(deta*deta+dphi*dphi);

      /*
      if (pfc.particleId() == 4 || pfc.particleId() == 5) {
        if (pfc.ecalEnergy()>0.0 && pfc.hcalEnergy()>0.0) {
        cout <<"Dr: " << dR << " Id: " << pfc.particleId() << " Eta: " << pfc.eta() << " Phi: " << pfc.phi()
             << " rawEcal: " << pfc.rawEcalEnergy() << " rawHcal: " << pfc.rawHcalEnergy()
             << " ecal: " << pfc.ecalEnergy() << " hcal: " << pfc.hcalEnergy()
             << " E: " << pfc.energy() << endl;
        }
      }
      */
      ...
      if (pfc.particleId() == 4 && dR < 0.2) {
        ...
        calo_    += pfc.ecalEnergy() + pfc.hcalEnergy();

        rawEcal_ += pfc.rawEcalEnergy();
        rawHcal_ += pfc.rawHcalEnergy();
        rawCalo_ += pfc.rawEcalEnergy() + pfc.rawHcalEnergy();

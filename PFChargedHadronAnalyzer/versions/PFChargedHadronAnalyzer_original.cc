...
PFChargedHadronAnalyzer::PFChargedHadronAnalyzer(const edm::ParameterSet& iConfig) : caloGeometryToken_(esConsumes()){
...
PFChargedHadronAnalyzer::analyze(const Event& iEvent, 
  ...
  auto caloGeometryHandle = iSetup.getHandle(caloGeometryToken_);
  //const CaloGeometry* pCalo = caloGeometryHandle.product();


   //edm::ESHandle<CaloGeometry> pCalo;
   //iSetup.get<CaloGeometryRecord>().get( pCalo );

   const CaloGeometry* theCaloGeom = &(*caloGeometryHandle);
